...
if($fetch_Data['new_arrival_status'] == 'Y'){
$sql_updateY=mysqli_query($conn,"UPDATE product SET new_arrival_status='N' WHERE product_id='$id'");
?>
<button type="button" class="btn btn-danger" onclick="change_newArrival(<?=$id?>)">Deactive</button>
<?php } else {
$sql_updateY=mysqli_query($conn,"UPDATE product SET new_arrival_status='Y' WHERE product_id='$id'");
?>
